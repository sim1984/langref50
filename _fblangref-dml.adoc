...
Оператор `INSERT` может включать необязательное предложение `RETURNING` для возврата значений из вставленной строки.
...
Оператор `INSERT ... VALUES` с предложением `RETURNING` всегда возвращает одну запись.
